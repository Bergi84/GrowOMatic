...
{
    gpio_set_function(aGpioPwm, GPIO_FUNC_PWM);
...
void gm_dcMotorCon::setGpioPwm(uint32_t aGpioPwm1, uint32_t aGpioPwm2)
{
    ...
    gpio_set_function(aGpioPwm2, GPIO_FUNC_SIO);
    mUni = false;
...
void gm_dcMotorCon::paraPwmDutyCb(void* aCbArg, TParaTable::paraRec_t* aPParaRec, bool aWrite)
{
    ...
    if(pObj->mUni)
    {
        ...
        pwm_set_chan_level(pObj->mPwmSlice, pObj->mPwmCh1, (aPParaRec->para * pObj->mPeriod)/1000);
    }
    else
    {
        if(locDuty > 0)
        ...
        else
        {
            ...
            pwm_set_chan_level(pObj->mPwmSlice, pObj->mPwmCh2, (aPParaRec->para * pObj->mPeriod)/1000);
        }
    }
}
